...
result_t SHT40_Init(void) {
	R_TRY(I2C_Write(I2C_Sensor, DEVICE_ADDRESS, SoftReset));
	CORE_Delay(2);
...
static result_t GetSensorData(SensorData_t *sensorData) {
    ...
    R_TRY(I2C_Write(I2C_Sensor, DEVICE_ADDRESS, Precision_High));
	CORE_Delay(10);
	...
	R_TRY(I2C_ReadBuffer(I2C_Sensor, DEVICE_ADDRESS, (uint8_t*)&raw_data, sizeof(raw_data)));
