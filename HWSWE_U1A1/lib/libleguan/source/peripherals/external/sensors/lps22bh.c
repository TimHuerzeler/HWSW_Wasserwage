...
result_t LPS22BH_Init(void) {
	...
	do {
		R_TRY(I2C_ReadRegister(I2C_Sensor, DEVICE_ADDRESS, WHO_AM_I | REQUEST_RESPONSE, &readDeviceId));
...
static result_t StartConversion(void) {
	...
	R_TRY(I2C_WriteRegister(I2C_Sensor, DEVICE_ADDRESS, CTRL_REG2, *(uint8_t*)&reg));

	uint8_t conversionDone = 0x00;
	do {
		R_TRY(I2C_ReadRegister(I2C_Sensor, DEVICE_ADDRESS, CTRL_REG2 | REQUEST_RESPONSE, &conversionDone));
...
result_t LPS22BH_GetTemperature(float32_t *value) {
	...
	R_TRY(I2C_ReadRegister(I2C_Sensor, DEVICE_ADDRESS, TEMP_OUT_L, &l));
	R_TRY(I2C_ReadRegister(I2C_Sensor, DEVICE_ADDRESS, TEMP_OUT_H, &h));
...
result_t LPS22BH_GetPressure(float32_t *value) {
    ...
    uint8_t xl, l, h;
	R_TRY(I2C_ReadRegister(I2C_Sensor, DEVICE_ADDRESS, PRESS_OUT_XL, &xl));
	R_TRY(I2C_ReadRegister(I2C_Sensor, DEVICE_ADDRESS, PRESS_OUT_L, &l));
	R_TRY(I2C_ReadRegister(I2C_Sensor, DEVICE_ADDRESS, PRESS_OUT_H, &h));
