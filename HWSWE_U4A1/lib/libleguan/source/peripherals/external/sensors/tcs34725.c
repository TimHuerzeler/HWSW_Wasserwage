...
static result_t SetIntegrationTime(IntegrationTime_t integrationTime) {
	R_TRY(I2C_WriteRegister(I2C_Sensor, DEVICE_ADDRESS, ATIME | COMMAND, integrationTime));

	return RESULT_SUCCESS;
}

static result_t SetGain(Gain_t gain) {
    R_TRY(I2C_WriteRegister(I2C_Sensor, DEVICE_ADDRESS, CTRL | COMMAND, gain));
...
static result_t Enable(void) {
	...
	R_TRY(I2C_WriteRegister(I2C_Sensor, DEVICE_ADDRESS, EN | COMMAND, *(uint8_t*)&reg));
...
static bool IsDone(void) {
	...
	if (I2C_ReadRegister(I2C_Sensor, DEVICE_ADDRESS, STATUS | COMMAND, &status) != RESULT_SUCCESS)
		return false;
...
result_t TC3A227E_GetColor(color_t *value) {
	...
	R_TRY(I2C_ReadRegister(I2C_Sensor, DEVICE_ADDRESS, RDATA  | COMMAND, &l));
	R_TRY(I2C_ReadRegister(I2C_Sensor, DEVICE_ADDRESS, RDATAH | COMMAND, &h));

	value->r = (h << 8) | l;

	R_TRY(I2C_ReadRegister(I2C_Sensor, DEVICE_ADDRESS, GDATA  | COMMAND, &l));
	R_TRY(I2C_ReadRegister(I2C_Sensor, DEVICE_ADDRESS, GDATAH | COMMAND, &h));

	value->g = (h << 8) | l;

	R_TRY(I2C_ReadRegister(I2C_Sensor, DEVICE_ADDRESS, BDATA  | COMMAND, &l));
	R_TRY(I2C_ReadRegister(I2C_Sensor, DEVICE_ADDRESS, BDATAH | COMMAND, &h));

	value->b = (h << 8) | l;

	R_TRY(I2C_ReadRegister(I2C_Sensor, DEVICE_ADDRESS, CDATA  | COMMAND, &l));
	R_TRY(I2C_ReadRegister(I2C_Sensor, DEVICE_ADDRESS, CDATAH | COMMAND, &h));
